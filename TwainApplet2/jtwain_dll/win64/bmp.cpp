...
void BMP_setImage(JNIEnv* env,jclass jclazz,jobject jimage, jboolean* hasException,const BITMAPINFOHEADER* bih,jbyteArray jbuf, jint jBytesPerLine){
  ...
  for(int i=bih->biHeight-1;i>=0;i--){
    if (bih->biBitCount!=32)
      ...
      env->SetByteArrayRegion(jbuf,joffset+1,jBytesPerLine-1,coffset); // for 32-bit convert BGRA (Windows BITMAPINFO RGBQUAD) to ABGR (Java BufferedImage)
...
jobject BMP_transfer32BitImage(JNIEnv* env, jclass jclazz,jboolean* hasException, const BITMAPINFOHEADER* bih){
  jobject jimage=(jobject)JNU_NewObject(
      env,hasException,
      TwainBufferedImage,
      "(III)V",
  ...
  if(jimage!=NULL){
    ...
    if(jbuf!=NULL){
      BMP_setImage(env,jclazz,jimage,hasException,bih,jbuf,bih->biWidth*4);
    }
  }
  return jimage;
}
...
//    fprintf(stderr,"bitCount=%d\ntype=%d\n",bih->biBitCount,bih->biCompression);
      ...
      if((type==BI_RGB)||(type==BI_BITFIELDS)){
        ...
        case 32: jimage=BMP_transfer32BitImage(env,jclazz,hasException,bih);break;
        default:
